...
export async function sendInsightDigest(email: string, name: string, insights: Insight[]) {
  ...
  try {
    await resend.emails.send({
      from: "RepWise Insights <andrew@tryrepwise.com>",
      to: email,
      subject: "Your top sales insights for today",
      html: `
<!DOCTYPE html>
<html>
<head>
...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
  <div style="max-width:560px;margin:0 auto;padding:40px 20px;">
    ...
    <div style="background:#111111;border:1px solid #222222;border-radius:16px;padding:32px;margin-bottom:24px;">
      <p style="color:#6b7280;font-size:13px;margin:0 0 8px 0;">Good morning ${name || "there"} 👋</p>
      <h1 style="color:#ffffff;font-size:22px;font-weight:700;margin:0 0 8px 0;">Your top insights today</h1>
      <p style="color:#9ca3af;font-size:14px;margin:0 0 24px 0;">Here are the most important things to act on right now.</p>

      ${topInsights.map(insight => `
        <div style="background:#0f0f0f;border:1px solid #1f2937;border-radius:12px;padding:16px;margin-bottom:12px;">
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
            <span style="background:${priorityColor(insight.priority)}20;color:${priorityColor(insight.priority)};font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;">${priorityLabel(insight.priority)}</span>
            <span style="color:#4b5563;font-size:11px;">${insight.category}</span>
            ${insight.metric ? `<span style="color:#60a5fa;font-size:11px;font-weight:600;margin-left:auto;">${insight.metric}</span>` : ""}
          </div>
      ...
      <a href="https://tryrepwise.com/dashboard" style="display:block;background:#3b82f6;color:#ffffff;text-align:center;padding:14px 24px;border-radius:12px;text-decoration:none;font-weight:700;font-size:14px;margin-top:16px;">
        View all insights →
      </a>
    </div>
