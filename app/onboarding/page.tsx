...
const challenges = [
  { id: "close_rate", label: "Low close rate", icon: "📉", desc: "Not converting enough contacts to sales" },
  { id: "follow_ups", label: "Missed follow-ups", icon: "📞", desc: "Leads going cold without follow-up" },
  { id: "territory", label: "Territory optimization", icon: "🗺️", desc: "Not knowing where to focus effort" },
  { id: "rep_performance", label: "Rep performance gaps", icon: "👥", desc: "Inconsistent performance across team" },
  { id: "data_visibility", label: "No data visibility", icon: "👁️", desc: "Flying blind without clear metrics" },
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center px-6 py-12">
          ...
          <div key={s.number} className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              <div className={"w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 " + (
                ...
                step === s.number ? "bg-blue-600 text-white ring-4 ring-blue-500/20" :
      ...
      <div className="w-full max-w-2xl bg-gray-900 border border-gray-800 rounded-3xl p-8 shadow-2xl">
        ...
        {step === 1 && (
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              {[
                { value: "rep" as Role, title: "Solo Rep", desc: "I want insights on my own performance", icon: "🎯", features: ["Personal insights", "Close rate tracking", "Territory analysis"] },
                { value: "manager" as Role, title: "Manager", desc: "I manage a team of reps", icon: "👔", features: ["Team insights", "Rep benchmarking", "Manager dashboard"] },
              ].map(option => (
                <button
                  key={option.value}
                  onClick={() => setRole(option.value)}
                  className={"p-5 rounded-2xl border text-left transition-all duration-200 hover:-translate-y-0.5 " + (role === option.value ? "bg-blue-600/10 border-blue-500 shadow-lg shadow-blue-500/10" : "bg-gray-800/50 border-gray-700 hover:border-gray-600")}
                >
            ...
            <button onClick={() => setStep(2)} disabled={!role} className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition shadow-lg shadow-blue-500/20">
              Continue
            </button>
          </div>
        )}
        ...
        {step === 2 && (
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
              {businessTypes.map(type => (
                <button
                  ...
                  className={"p-4 rounded-2xl border text-left transition-all duration-200 hover:-translate-y-0.5 " + (businessType === type.id ? "bg-blue-600/10 border-blue-500 shadow-lg shadow-blue-500/10" : "bg-gray-800/50 border-gray-700 hover:border-gray-600")}
                >
                  <div className="text-2xl mb-2">{type.icon}</div>
                  <h3 className="text-white font-bold text-xs mb-1">{type.label}</h3>
                  <p className={"text-xs leading-relaxed " + (businessType === type.id ? "text-blue-200/70" : "text-gray-600")}>{type.desc}</p>
                </button>
              ))}
            </div>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setStep(1)} className="flex-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition">
                Back
              </button>
              <button onClick={() => setStep(3)} disabled={!businessType} className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition shadow-lg shadow-blue-500/20">
                Continue
              </button>
            </div>
          </div>
        ...
        {/* Step 3 — Team size and challenge */}
        {step === 3 && (
          <div>
              ...
              <div className="grid grid-cols-4 gap-3">
                {teamSizes.map(size => (
                  <button
                    ...
                    className={"p-3 rounded-xl border text-center transition-all " + (teamSize === size.id ? "bg-blue-600/10 border-blue-500" : "bg-gray-800/50 border-gray-700 hover:border-gray-600")}
                  >
              ...
              <div className="grid grid-cols-2 gap-3">
                {challenges.map(challenge => (
                  <button
                    ...
                    className={"flex items-center gap-3 p-3 rounded-xl border text-left transition-all " + (mainChallenge === challenge.id ? "bg-blue-600/10 border-blue-500" : "bg-gray-800/50 border-gray-700 hover:border-gray-600")}
                  >
                    <span className="text-xl">{challenge.icon}</span>
                    <div>
                      <p className="text-white text-xs font-semibold">{challenge.label}</p>
                      <p className={"text-xs " + (mainChallenge === challenge.id ? "text-blue-200/70" : "text-gray-600")}>{challenge.desc}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setStep(2)} className="flex-1 bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition">
                Back
              </button>
              <button onClick={() => setStep(4)} disabled={!teamSize || !mainChallenge} className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white font-semibold px-6 py-3.5 rounded-xl text-sm transition shadow-lg shadow-blue-500/20">
                Continue
              </button>
            </div>
          </div>
          ...
          <div>
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400 text-sm leading-relaxed">
                Your AI insights will be tailored to <span className="text-white font-semibold">{businessTypes.find(b => b.id === businessType)?.label}</span> sales with a focus on <span className="text-white font-semibold">{challenges.find(c => c.id === mainChallenge)?.label.toLowerCase()}</span>.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setStep(3)} className="bg-gray-800 hover:bg-gray-700 border border-gray-700 text-white font-semibold px-4 py-3.5 rounded-xl text-sm transition">
                Back
              </button>
              <button
