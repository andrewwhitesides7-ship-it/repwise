...
export async function inviteRep(formData: FormData) {
  ...
  await resend.emails.send({
    from: "Adunda <onboarding@resend.dev>",
    to: email.trim(),
    subject: `${profile.full_name || "Your manager"} invited you to join Adunda`,
    html: `
      <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 40px 20px;">
        ...
        <p style="color: #374151; font-size: 16px; margin-top: 24px;">
          <strong>${profile.full_name || "Your manager"}</strong> has invited you to join <strong>${team?.name || "their team"}</strong> on Adunda.
        </p>
        <p style="color: #6b7280; font-size: 14px;">
          Adunda is a sales intelligence platform that analyzes your activity and tells you exactly where you are losing deals.
        </p>
